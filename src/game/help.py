...
class HelpScene(Scene):
    ...
    def __init__(self, parent):
        ...
        # Load all monster images from sprite sheet, categorized as acid1, acid2, base1, base2, salt
        self.animations = load_sprite_sheet("assets/images/enemy/monsters.png",
                                            5, 4, directions=("a1", "a2", "b1", "b2", "s"), scale=1)
        # Add the last frames of salt and base to the metal frame list
        metal_frame = [
    ...
    def render_rule(self, screen: pygame.Surface):
        ...
        if self.line_surfaces_left:
            for line_surface in self.line_surfaces_left:
                ...
                _y += line_surface.get_height() + 5  # 5 pixel line spacing
        _y = 220
        ...
        if self.line_surfaces_right:
            for line_surface in self.line_surfaces_right:
                screen.blit(line_surface, (_x, _y))
                _y += line_surface.get_height() + 4  # 4 pixel line spacing
    ...
    def _render_monster_group(self, screen: pygame.Surface, x: int, y: int, title: str, color: pygame.Color,
                              group_names: list[str]):
        """Render monster group"""
    ...
    def render(self, screen: pygame.Surface):
        """Render the help scene to the screen.
    ...
    def process_input(self, event: pygame.event.Event):
        """Process user input events for the help scene.
